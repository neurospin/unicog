...
%% Added by A. Moreno - 04/07/2016
% To improve figures visualization
plot(t,pulset, 'LineWidth', linewidth); 
...
if resp_max < inf 
    ...
    %% Added by A. Moreno - 04/07/2016
    % To improve figures visualization
    plot(t, ones(size(t)) * resp_max, 'k--', 'LineWidth', linewidth);
    
    hold on; 
    %hp = plot(t, -ones(size(t)) * resp_max, 'k--');        
    %% Added by A. Moreno - 04/07/2016
    % To improve figures visualization
    hp = plot(t, -ones(size(t)) * resp_max, 'k--', 'LineWidth', linewidth);        
...
%% Added by A. Moreno - 04/07/2016
% To improve figures visualization
hs(3) = subplot(2,2,3); plot(rout, [feqht, cos(feqht), sin(feqht)], 'LineWidth', linewidth); 

xlabel('normalized breathing amplitude'); 
%title(...
%    'equalized histogram bin amplitude -> phase transfer function (f_{eqht})');
title({'equalized histogram bin amplitude', '-> phase transfer function (f_{eqht})'}); % A. Moreno 02/08/2016
legend('f: normalized amplitude -> phase transfer function', 'cos(f)', ...
    'sin(f)', 'Location', 'best'); % A. Moreno 02/08/2016
...
%% Added by A. Moreno - 04/07/2016
% To improve figures visualization
plot(t, [npulse*10, dpulse, (rphase-pi)], 'LineWidth', linewidth);

%legend('10*normalized breathing belt amplitude', ...
%    '-1 = exhale, 1 = inhale', 'estimated respiratory phase');    
legend('10*normalized breathing belt amplitude', ...
    '-1 = exhale, 1 = inhale', 'estimated respiratory phase','Location','best');  % A. Moreno 02/08/2016   
ylim([-0.2 10.2]);
%title('Histogram-based respiration phase estimation');
title({'Histogram-based', 'respiration phase estimation'}); % A. Moreno 02/08/2016
