function [c, r, t, cpulse, verbose] = tapas_physio_read_physlogfiles_siemens_minn_tics(...
    log_files, verbose)

%% Adapted from function "tapas_physio_read_physlogfiles_siemens_tics" by A. Moreno - 01/08/2016
...
%% Added by A. Moreno - 01/08/2016
% Find value of "log_files.relative_start_acquisition"

fid = fopen(log_files.scan_timing);
%C = textscan(fid, '%d %d %d %d', 'HeaderLines', 1);
C = textscan(fid, '%d %d %d %d', 'HeaderLines', 9);
...
log_files.relative_start_acquisition = (double(ticsAcq(1))) * dtTics;
...
if hasCardiacFile
    ...
    %% Added by A. Moreno - 04/07/2016
    % To improve figures visualization
    plot(tCardiac-tOffset, c, 'r.-', 'LineWidth', linewidth); hold all;
...
if hasRespirationFile
    ...
    %% Added by A. Moreno - 04/07/2016
    % To improve figures visualization
    plot(tRespiration-tOffset, r, 'g.-', 'LineWidth', linewidth);
