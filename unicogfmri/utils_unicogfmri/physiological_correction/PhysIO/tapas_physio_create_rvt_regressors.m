...
%
% $Id: tapas_physio_create_rvt_regressors.m 807 2015-08-11 23:55:12Z kasperla $
...
if verbose.level >=2
    ...
    plot(sample_points,rvt, 'g','LineWidth',linewidth);xlabel('time (seconds)');
...
if verbose.level >= 2
    ...
    %% Changed by A. Moreno - 23/07/2016
    plot(t, rrf,'g','LineWidth',linewidth);xlabel('time (seconds)');
...
if verbose.level >= 2
    ...
    %% Changed by A. Moreno - 23/07/2016
    plot(sample_points, convRVT,'g','LineWidth',linewidth);xlabel('time (seconds)');
...
if verbose.level >= 2
    ...
    %% Changed by A. Moreno - 23/07/2016
    hp{1} = plot(samplePointsOut, rvtOut,'k--','LineWidth',linewidth);hold all;
    hp{2} = plot(samplePointsOut, squeeze(convRVTOut(:,iShiftMin,:)),'g','LineWidth',linewidth);
    xlabel('time (seconds)');
    title('RVT regessor');
