...
if verbose.level>=2
    ...
    %% Changed by A. Moreno - 23/07/2016
    hp(1) = plot(t,fr, 'g','LineWidth',linewidth); hold all;
    stem(timeRpulseMax, maxFr*ones(nMax,1),'b','LineWidth',linewidth);
    stem(timeRpulseMin, -maxFr*ones(nMin,1),'r','LineWidth',linewidth);
    hp(2) = plot(t, ampRpulseMax, 'b','LineWidth',linewidth);
    hp(3) = plot(t, ampRpulseMin, 'r','LineWidth',linewidth);
    hp(4) = plot(t, interpDurationBreath, 'm','LineWidth',linewidth);
...
switch method
    ...
    case 'linear'
        ...
        if verbose.level>=2
            ...
            %% Changed by A. Moreno - 23/07/2016
            hp(end+1) = plot(t, interpRv,'k','LineWidth',linewidth);
            hp(end+1) = plot(t, interpRvt, 'c','LineWidth',linewidth);
...
if verbose.level >=2
    ...
    %% Changed by A. Moreno - 23/07/2016
    hp(end+1) = plot(sample_points,rv,'k+','LineWidth',linewidth);
    hp(end+1) = plot(sample_points,rvt,'cd','LineWidth',linewidth);
