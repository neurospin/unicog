function [VOLLOCS, LOCS, verbose] = ...
    tapas_physio_create_scan_timing_from_tics_siemens_minn(t, log_files, verbose)
...
C = textscan(fid, '%d %d %d %d', 'HeaderLines', 9); % Adapted by A. Moreno - 01/08/2016
...
tAcqSeconds     = (double(ticsAcq) - double(ticsAcq(1))) * dtTicSeconds;
%log_files.relative_start_acquisition = (double(ticsAcq(1))) * dtTics; % IMPORTANT!!!
...
%    %% Added by A. Moreno - 04/07/2016
%    % To improve figures visualization
%    plot(t, c, 'k', 'LineWidth', linewidth);
