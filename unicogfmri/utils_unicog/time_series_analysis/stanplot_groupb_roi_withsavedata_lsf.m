function [meany,meanY,sey,seY,firy,firY,firciy,firciY]=stanplot_groupb_roi_withsavedata_lsf(Finterest,groupvar,timeaxis,baselinepoints,marsbarroi,name,dirname)
...
for g=1:ngroups
    ...
    for sub=startnumber+(1:nsub(g))
        ...
        [smeany,smeanY,ssey,sseY,sfiry,sfirY,sfirciy,sfirciY]=stanplot_singleroi2(modeldir,Finterest,groupvar,timeaxis,baselinepoints,marsbarroi,name);
        if any(~isnan(smeany(:)))
        ...
        fname = sprintf('%s/stanplot_%s_g%d_s%02d.txt',dirname,name,g,sub)
        save(fname, 'smeany','-ASCII')
    end
...
%%    plot(timeaxis,meanY{g},'-','LineWidth',3); %%% model
    ...
    s=sprintf('Group "%s" (n=%d) %s',groupn{g},nsub(g),XYZstr);
...
%    plot(timeaxis,firY{g},'-','LineWidth',3); %%% model
    ...
    s=sprintf('FIR. Group "%s" (n=%d) %s',groupn{g},nsub(g),XYZstr);
    s = strrep(s,'_','-');
    title(s);
